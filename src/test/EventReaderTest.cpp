...
class EventReaderTest : public testing::Test {
    ...
    void SetUp() {
        event_reader = std::make_unique<EventReader<BlockingQueue<dvs128::Event>>>();
    }

public:
    std::unique_ptr<EventReader<BlockingQueue<dvs128::Event>>> event_reader;
};
...
{
    ASSERT_EQ(event_reader->isBufferSet(), false);
    event_reader->setOutputBuffer(std::make_shared<BlockingQueue<dvs128::Event>>());
